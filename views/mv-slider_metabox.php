<?php
//gets all info related to current post
  $meta = get_post_meta( $post->ID );

// gets specific keys associated with current post
  $link_text = get_post_meta( $post->ID, 'mv_slider_link_text', true );
  $link_url = get_post_meta( $post->ID, 'mv_slider_link_url', true );

// displays everything associated with inputted variable on the DOM
  //var_dump( $meta )
?>

<table class="form-table mv-slider-metabox">
  <input type="hidden" name="mv_slider_nonce" value="<?php echo wp_create_nonce( 'mv_lider_nonce' ) ?>">
  <tr>
    <th>
  ...
  <tr>
    <th>
      <label for="mv_slider_link_url">Link Text</label>
    </th>
    <td>
      <input
        type="url"
