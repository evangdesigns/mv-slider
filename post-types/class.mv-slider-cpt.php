...
if( !class_exists( 'MV_Slider_Post_Type')) {
  class MV_Slider_Post_Type{
    ...
    //BEGIN GAURD CLAUSES
      ...
      //checks if Autosave is active
      if( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ){
        return;
      }
      //check the current user has the ability to edit the page and or post based on post type
    ...
    //END GAURD CAUSES
      if( isset( $_POST['action'] ) && $_POST['action'] == 'editpost' ) {
        ...
        if( empty( $new_link_text )){
           update_post_meta( $post_id, 'mv_slider_link_text', 'Please add a Button Text');
        } else {
           update_post_meta( $post_id, 'mv_slider_link_text', sanitize_text_field( $new_link_text ), $old_link_text );
        }

        if( empty( $new_link_url )){
          update_post_meta( $post_id, 'mv_slider_link_url', 'Please add a url');
        } else {
